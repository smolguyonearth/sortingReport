...
void comparison_counting_sort(int arr[], int size) {
    int *count = (int *)calloc(size, sizeof(int)); // Initialize count array with zeros
    int *sorted = (int *)malloc(sizeof(int) * size); // Array to store sorted elements

    // Compute the count of elements less than each element
    ...
    // Place elements in sorted order based on count
    for (int i = 0; i < size; i++) {
...
int main() {
    ...
    const char *log_files[] = {"comparison_count_log_original.txt", "comparison_count_log_mintomax.txt", "comparison_count_log_maxtomin.txt"};
    ...
    for (int mode = 0; mode < 3; mode++) { // Loop through original, sorted, and reversed cases
        ...
        for (int i = 0; i < num_sizes; i++) {
            ...
            comparison_counting_sort(arr, size);
